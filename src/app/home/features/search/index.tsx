...
export const Search = ({ id }: { id?: string }) => {
  const [
    ...
    SearchSelectors.getAllSearchResults(s),
    SearchSelectors.getLastResultStatus(s),
    SearchSelectors.getSearchResultLoading(s),
    s.createResultFromHistory,
    s.id,
  ]);
  const [refreshHistory, getAnswerFromHistory] = useHistoryStore((s) => [
    s.initializeHistory,
    s.getAnswerById,
  ]);
      ...
      <Flex vertical className={styles.child}>
        {allSearchResults.map((result, index) => (
          <SearchResult
            ...
            key={result.id}
            result={result}
            isCurrent={index === allSearchResults.length - 1}
          />
        ))}
        <FollowupSearch />
      </Flex>
